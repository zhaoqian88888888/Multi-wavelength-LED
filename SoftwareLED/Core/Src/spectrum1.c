...
void update_spectrum_array_1(float I, float T, const uint16_t *X_array1, float *Y_array1, uint16_t size) {
    float a = calculate_a_1(I, T);
    float b = calculate_b_1(I, T);
    float A2 = calculate_A2_1(I, T);
    float x1 = calculate_x1_1(I, T);
		float c  = calculate_c_1(I, T);

    for (uint16_t i = 0; i < size; i++) {
        float X = (float)X_array1[i];
        float part1 = 1 * expf((x1 - X) / a);
        float part2 = A2 * expf((X - x1) / b);
        Y_array1[i] = c / (part1 + part2);
    }
}

float Y_array1[NUM_POINTS];
float Z_array1[NUM_POINTS];
float Y_integral1 = 0.0f;
float Z_integral1 = 0.0f;


const uint16_t X_array1[NUM_POINTS] = {300, 301, 302, 303, 304, 305, 306, 307, 308, 309, 310, 311, 312, 
...
const float Excitation1[NUM_POINTS] = {
...
const float Excitation1_T[NUM_POINTS] ={
0                 ,
0                 ,
0                 ,
0                 ,
...
float Y_array1[NUM_POINTS];//300-750的光谱强度
float y_result1, z_result1;

void update_and_use_spectrum_1(float intensity, float temperature) {
    update_spectrum_array_1(intensity, temperature, X_array1, Y_array1, NUM_POINTS);
    // Y_array 现在包含了新的光谱数据，可供后续处理

		compute_integrals_1(&y_result1, &z_result1);//打印y光强，z荧光信号强度
}
...
void compute_integrals_1(float *y_result1, float *z_result1) {
    ...
    for (int i = 1; i < NUM_POINTS; i++) {
        Z_array1[i] = Y_array1[i] * Excitation1[i]* Excitation1_T[i] / 100;
        Y_integral1 += (Y_array1[i] + Y_array1[i-1])  * 0.5f;
        Z_integral1 += (Z_array1[i] + Z_array1[i-1])  * 0.5f;
    }
    
    *y_result1 = Y_integral1;
    *z_result1 = Z_integral1;
}

...
void update_spectrum_array(float I, float T, const uint16_t *X_array, float *Y_array, uint16_t size) {
    float a = calculate_a(I, T);
    float b = calculate_b(I, T);
   // float A1 = calculate_A1(I, T);
    float A2 = calculate_A2(I, T);
    float x1 = calculate_x1(I, T);
		float c  = calculate_c(I, T);

    for (uint16_t i = 0; i < size; i++) {
        float X = (float)X_array[i];
        float part1 =  expf((x1 - X) / a);
        float part2 = A2 * expf((X - x1) / b);
        Y_array[i] = c / (part1 + part2);
    }
}

float Y_array[NUM_POINTS];
float Z_array[NUM_POINTS];
float Y_integral = 0.0f;
float Z_integral = 0.0f;

const uint16_t X_array[NUM_POINTS] = {300, 301, 302, 303, 304, 305, 306, 307, 308, 309, 310, 311, 312, 
...
const float Excitation[NUM_POINTS] ={
...
const float Excitation_T[NUM_POINTS] ={
0                 ,
0                 ,
0                 ,
0                 ,
...
float Y_array[NUM_POINTS];//300-750的光谱强度
float y_result, z_result;

void update_and_use_spectrum(float intensity, float temperature) {
    update_spectrum_array(intensity, temperature, X_array, Y_array, NUM_POINTS);
    // Y_array 现在包含了新的光谱数据，可供后续处理

		compute_integrals(&y_result, &z_result);//打印y光强，z荧光信号强度
}
...
void compute_integrals(float *y_result, float *z_result) {
    ...
    for (int i = 1; i < NUM_POINTS; i++) {
        Z_array[i] = Y_array[i] * Excitation[i] * Excitation_T[i]/ 100;//* Excitation_T[i]
        Y_integral += (Y_array[i] + Y_array[i-1])  * 0.5f;
        Z_integral += (Z_array[i] + Z_array[i-1])  * 0.5f;
    }
